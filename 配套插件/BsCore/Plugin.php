   ...
   \Typecho\Plugin::factory('Widget_Upload')->modifyHandle = [__CLASS__, 'bd_modifyHandle'];
    ...
    \Typecho\Plugin::factory('Widget_Upload')->attachmentDataHandle = [__CLASS__, 'bd_attachmentDataHandle'];
       ...
       //\Typecho\Plugin::factory('admin/header.php')->header_1081 = [__CLASS__, 'loginverify'];
        ...
        \Typecho\Plugin::factory('Widget_Comments_Edit')->mark_1 = [__CLASS__,'parseCommentMark'];
         }
...
if (strpos($db->fetchAll($db->query("select sql from sqlite_master where type = 'table' and name = 'typecho_bscore_friendlinks'"))[0]['sql'], 'type') == false) {
            $db->query('ALTER TABLE `' . $prefix . 'bscore_friendlinks` ADD `type` TEXT NOT NULL  DEFAULT 0;');
        }
if (strpos($db->fetchAll($db->query("select sql from sqlite_master where type = 'table' and name = 'typecho_bscore_friendlinks'"))[0]['sql'], 'rejectreason') == false) {
...
<link rel="stylesheet" href="//lib.baomitu.com/limonte-sweetalert2/11.10.1/sweetalert2.min.css" />
<link href="<?php echo $dir;?>sweetalert2/bootstrap-4.css" rel="stylesheet">
<link rel="stylesheet" href="//lib.baomitu.com/bootstrap-colorpicker/3.4.0/css/bootstrap-colorpicker.min.css" />
<script src="//lib.baomitu.com/limonte-sweetalert2/11.10.1/sweetalert2.min.js"></script>
        <script type="text/javascript" src="//lib.baomitu.com/bootstrap-colorpicker/3.4.0/js/bootstrap-colorpicker.min.js"></script>
          	...
          	$('#wmd-button-row').append('<li class="wmd-button" id="wmd-mp3-button" title="插入mp3音频文件"><span style="background: none;margin-top:5px;font-size: large;text-align: center;color: #999999;font-family: serif;"><i class="fa fa-file-sound-o"></i></span></li>');
				if($('#wmd-button-row').length !== 0){
					$('#wmd-mp3-button').click(function(){
						var rs = '[bsmp3 url="音频文件地址" singer="歌手" name="歌曲名" image="歌曲封面图地址"]\n';
						mp3(rs);
					})
				}
...
<script src="https://cdn.bootcdn.net/ajax/libs/turndown/7.1.2/turndown.min.js"></script>
<style>
...
editor.codemirror.doc.replaceSelection(fileurl);
    ...
    editor.codemirror.doc.replaceSelection('[bsplayer url="视频文件地址" image="视频封面图地址"]'); 
            ...
            function mp3(editor){
    editor.codemirror.doc.replaceSelection('[bsmp3 url="音频文件地址" singer="歌手" name="歌曲名" image="歌曲封面图地址"]'); 
            }
    ...
    editor.codemirror.doc.replaceSelection('[e]'); 
            }
...
$comment['url'] =htmlspecialchars($comment['url'],ENT_QUOTES,'UTF-8');
 ...
 if(!$_POST['vtype'] && $_POST['vtype'] !== 'cross' && $_POST['vtype'] !== 'friendcircle'){ //判断POST过来的vtype字段是否为指定类型或为空
...
if(!$response->result){
 ...
 $select = $db->fetchRow($db->select()->from('table.comments')->where('type = ?', 'comment')->order('coid', \Typecho\Db::SORT_DESC));
