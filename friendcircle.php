...
<link href="<?php AssetsDir();?>assets/vendors/friendcircle/friendcircle.css?v=11" rel="stylesheet">
<link href="<?php AssetsDir();?>assets/vendors/friendcircle/plyr.min.css" rel="stylesheet">
<script>
<?php if(Bsoptions('fcircle_hiddenMode') == true || Bsoptions('site_style') == '2'):?>
   setting.ClockModule = 'false';
   <?php else:?>
   setting.ClockModule = 'true';
   <?php endif;?>
</script>
<?php
$parameter= array(
...
;
  ...
  $(".plyr--video .plyr__controls .plyr__progress__container").show();
  $(".plyr--video .plyr__controls .plyr__time").show();
});
         player.on('exitfullscreen', (event) => {
  const instance = event.detail.plyr;
  $(".plyr--video .plyr__controls .plyr__progress__container").hide();
  $(".plyr--video .plyr__controls .plyr__time").hide();
});
});
}
plyr();
          <?php if(Bsoptions('Emoji') == false) :?>
          $.getScript(setting.import+"assets/vendors/bs-emoji/bs-emoji.js?v=1.4",function(){
...
if (preg_match("/^([0-9]+)([a-z]{1,2})$/i", $phpMaxFilesize, $matches)) {
	...
	var uploader_video = new plupload.Uploader({
            ...
            filters         :   {
                max_file_size       :   '<?php echo $phpMaxFilesize;?>',
                mime_types          :   [{'title' : '允许上传的文件', 'extensions' : 'mp3,mp4,mov,wmv,wma,rmvb,rm,avi,flv,ogg,oga,ogv,flac'}],
                prevent_duplicates  :   false
            },
				multi_selection: false, 
				init: {
